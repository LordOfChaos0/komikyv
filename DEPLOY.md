...
| Письма не приходят | SMTP не настроен / ошибка отправки | Диагностика по §14: `node scripts/smtp-check.js` |
...
## 14. Диагностика SMTP (почта не отправляется)

Почта в проекте используется в двух местах: подтверждение email при
регистрации и восстановление пароля. Оба идут через `src/lib/mailer.ts`
(nodemailer + Яндекс SMTP). Если письма не приходят — идите по шагам.

### 14.1. Шаг 1: что говорит лог сервиса (30 секунд)

```bash
sudo journalctl -u komikyv -n 300 --no-pager | grep -e "DEV MODE" -e "Email send error"
```

| Что видно в логе | Что это значит |
|---|---|
| `📧 [DEV MODE] Verification code …: 123456` | **SMTP не настроен** — приложение не видит `SMTP_*` в env, письма не отправляются вообще, код печатается в лог |
| `Email send error: …` | SMTP настроен, но отправка падает — см. расшифровку ниже |
| Ничего | Отправки не было с момента перезапуска — зарегистрируйтесь / запросите сброс пароля и повторите |

### 14.2. Шаг 2: скрипт полной диагностики

Из корня репозитория на VM:

```bash
node scripts/smtp-check.js            # проверит всё, без отправки писем
node scripts/smtp-check.js --send     # то же + тестовое письмо самому себе
```

Скрипт по шагам: находит и **сравнивает `.env` и `.next/standalone/.env`**
(приложение читает только второй — частая причина «настроил, но не
работает»), проверяет DNS и TCP-порт, показывает SMTP-баннер, пробует
логин/пароль и расшифровывает ошибки человеческим языком.

### 14.3. Расшифровка типичных ошибок

| Ошибка | Причина | Решение |
|---|---|---|
| `EAUTH 535 5.7.8 authentication failed` | Яндекс отклонил логин/пароль | 1) включить SMTP: mail.yandex.ru → Настройки → «Почтовые программы»; 2) использовать **пароль приложения** (id.yandex.ru → Безопасность), не обычный; 3) один раз войти в веб-почту; 4) свежесозданный пароль подождать ~10 мин |
| `ETIMEDOUT` / порт не отвечает | Хостер блокирует исходящий SMTP (анти-спам) | `SMTP_PORT=587` (STARTTLS поддерживается кодом автоматически); если оба закрыты — тикет хостеру |
| `553 5.7.1 Sender address rejected: user not found` (и 550/554) | Конверт отклонён: `SMTP_FROM` не совпадает со `SMTP_USER` — типичный случай: пример `noreply@komikyv.ru` из `.env.example`, такого ящика не существует | `SMTP_FROM="Коми кыв <user@yandex.ru>"` (тот же адрес, что `SMTP_USER`) или удалить строку. Свежий код для Яндекса делает это сам (в логе будет предупреждение) |
| Письмо ушло, но не приходит | Спам / получатель | Проверить папку «Спам»; адреса на домене `.рф` многие провайдеры не принимают |

### 14.4. Ловушка со standalone-.env (главная причина «настроил — не работает»)

Сервис работает из `.next/standalone` и читает **только
`.next/standalone/.env`**, а не корневой `~/komikyv/.env`. Деплой
пересоздаёт standalone-файл из репо-`.env`. Поэтому после правки SMTP
в корневом `.env`:
...
Скрипт `scripts/smtp-check.js` показывает расхождение этих двух файлов
отдельным шагом.
