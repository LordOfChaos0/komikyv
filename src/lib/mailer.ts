...
export async function sendPasswordResetEmail(
  ...
  if (!config) {
    // SMTP не настроен — возвращаем код для dev-режима
    console.log(`📧 [DEV MODE] Password reset code for ${toEmail}: ${code}`);
    return { sent: false, devCode: code };
  }
  ...
  try {
    ...
    const html = `
      <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
        ...
        <div style="background: #f8f9fa; padding: 30px; border-radius: 0 0 12px 12px; border: 1px solid #e0e0e0;">
          <h2 style="color: #2d6a4f; margin-top: 0;">Восстановление пароля</h2>
          <p style="color: #333; font-size: 16px;">Здравствуйте${userName ? `, ${userName}` : ""}!</p>
          <p style="color: #555; font-size: 15px; line-height: 1.6;">
            Кто-то (надеемся, вы) запросил сброс пароля к вашему аккаунту
            на платформе «Коми кыв». Для установки нового пароля введите этот код:
          </p>
          <div style="text-align: center; margin: 30px 0;">
            <div style="display: inline-block; background: #2d6a4f; color: white; font-size: 36px; font-weight: bold; letter-spacing: 8px; padding: 20px 40px; border-radius: 12px;">
              ${code}
    ...
    const text = `Коми кыв — восстановление пароля
...
© ${new Date().getFullYear()} Коми кыв`;
    ...
    await transporter.sendMail({
      from: config.from,
      to: toEmail,
      subject: "Коми кыв — код восстановления пароля",
      text,
      html,
    });
