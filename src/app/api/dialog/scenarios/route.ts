...
export async function GET() {
  ...
  // Prisma groupBy: вывод типов через const + тернарник (без явной аннотации,
  // которая ломает контекстную типизацию перегрузок groupBy)
  const sessions = user
    ? await db.dialogSession.groupBy({
        by: ["scenarioId"],
